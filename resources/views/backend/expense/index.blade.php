            ...
            <table id="company-list" class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                @foreach($expe as $ex)
                    <tr>
                        ...
                        <td>{{ $ex->ex_date }}</td>
                        <td>
                            <button type="button" class="expense_modal btn btn-primary" data-toggle="modal" data-target="#expenseEditModal" data-id="{{ $ex->id }}">Edit</button>
                            {!! Form::open(['route' => ['expenses.destroy', $ex->id ], 'method' => 'DELETE', 'class'=>'delete_form', 'style'=>'display:inline' ])!!}
